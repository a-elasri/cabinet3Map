...
import 'package:cabinet3/main.dart';
...
Future<Map> getDirectionsAPIResponse(LatLng currentLatLng, int index) async {
  final response = await getCyclingRouteUsingMapbox(
      currentLatLng,
      LatLng(double.parse(cabinets[index].coordinates['latitude']!),
          double.parse(cabinets[index].coordinates['longitude']!)));
  Map geometry = response['routes'][0]['geometry'];
  num duration = response['routes'][0]['duration'];
  num distance = response['routes'][0]['distance'];
  print('-------------------${cabinets[index].name}-------------------');
  print(distance);
  print(duration);
...
void saveDirectionsAPIResponse(int index, String response) {
  sharedPreferences.setString('cabinet--$index', response);
}